...
public interface Crypto2HRepository extends JpaRepository<Crypto2H, Long>, JpaSpecificationExecutor<Crypto2H> {
    ...
    @Transactional
    @Modifying
    @Query("UPDATE Crypto2H c SET c.nextDayMaxPrice = :maxValue WHERE c.symbol = :symbol AND c.createdAt > :date c.createdAt < :to AND c.nextDayMaxPrice < :maxValue")
    ...
    @Query("SELECT COUNT(*) FROM Crypto2H c WHERE c.cryptoType = :cryptoType AND c.createdAt > :startDate  AND c.createdAt < :endDate")
    double findAllStats(@Param("cryptoType") CryptoType cryptoType,
                        @Param("startDate") Long startDate,
                        @Param("endDate") Long endDate);

    @Query("SELECT COUNT(*) FROM Crypto2H c WHERE c.cryptoType = :cryptoType AND c.createdAt > :startDate  AND c.createdAt < :endDate AND c.nextDayMaxPrice >= c.priceToSell")
